	...
	function getListing (query, cb) {
		...
		db.get.listing(query, function (err, doc) {
			if (err || !doc.length) {
				...
				// FIXME
				doc[0].asset = 'http://'+host+'/assets/asset/' + doc[0]['assetId'];
				doc[0].id = 'http://'+host+'/listings/listing/' + doc[0]['_id'];
				cb(doc[0] || errMsg);
			}
		});
	}
	...
	function getAsset (query, cb) {
		...
		db.get.asset(query, function (err, doc) {
			if (err || !doc.length) {
				...
				doc[0].assetContent = 'http://'+host+'/asset/content/' + doc[0]['_id'];
				doc[0].id = 'http://'+host+'/assets/asset/' + doc[0]['_id'];
				cb(doc[0] || errMsg);
			}
		});
	}
