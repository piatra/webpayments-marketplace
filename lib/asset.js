...
module.exports = function (options) {
	...
	function getUserAssets (query, cb) {
	  var docs = [];
	  console.log('getUserAssets', query);

	  if (isEmpty(query)) cb({error: 'Empty query', query: query}, []);
	  else
	...
	function get (sel, query, cb) {

		var getDoc = function (err, doc) {
			if (err) cb(err);
			if (!doc.length) cb({err:'error', msg:'not found', query:query});
			cb(null, doc[0]);
		}
		
		if (sel.listing)
			db.get.listing(query, getDoc);
		if (sel.asset)
			db.get.asset(query, getDoc);
	...
	function replaceListingAndAsset (graph, cb) {
		removeAll({
			id: graph['@graph'][0].listingId
		}, function () {
			saveAssetAndListing(graph, cb);
		})
	}

	function removeAll (query, cb) {
		console.log('remove all', query);
	...
	assetUtils.replaceListingAndAsset = replaceListingAndAsset;
