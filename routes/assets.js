...
var assetParser = require('../lib/assetParser.js');
...
function generatePayee (id, req, type) {

   if (!Array.isArray(req.destination))
      req.destination = [req.destination];
   if (!Array.isArray(req.price))
      req.price = [req.price];

 if (type == 'listing') {
    return req.destination.map(function (d, idx) {
       return {
          id: HOST + '/listings/listing/' + id + '/payee',
          type: 'Payee',
          destination: d,
          currency: 'USD',
          payeeGroup: ['vendor'],
          payeeRate: req.price[idx],
...
var assets = {
  ...
  createAssetAndListing: function (req, res) {
    ...
    async.waterfall([
      ...
      function(keysPair, callback) {
        ...
        var payee = generatePayee(assetId, req.body, 'asset');
      ...
      function(signedAsset, assetHash, callback) {
        ...
        var assetUrl = HOST + '/assets/asset/' + assetId;
        var payee = generatePayee(assetId, req.body, 'listing');
  ...
  purchase: function (req, res) {
     // this is a big FIXME
      //if (!req.session.identity) {
        ...
        // return res.end('No identity found');
      //}
                 ...
                 request.get({url : HOST + '/listings/listing/' + id, json: true}, function (err, resp, listing) {
                    keys.getKeyPair(function (err, data) {
                       if (err) {
                          ...
                          console.log('https://dev.payswarm.com/i/piatra');
                          console.log(data.publicKey.id);
                          console.log(data.publicKey.privateKeyPem);
                          console.log('----');
                          payswarm.purchase(listing, {
                             transactionService: authority + 'transactions',
                             customer: 'https://dev.payswarm.com/i/piatra',
  ...
  */
        ...
        if (~resp.preferences.indexOf('PreAuthorization')) {
          async.waterfall([
            function (callback) {
             ...
             console.log(req.session);
              user.get({preferences:1}, { email: req.cookies.email }, function (err, doc) {
                console.log(doc);
                if (err) {
                  callback(err);
                } else {
  ...
  myAssets: function (req, res) {
    ...
    asset.getUserAssets(query, function (err, assets) {
      if (err) {
        console.log('error asset.js', err);
  ...
  addToCart: function (req, res) {
    ...
    // extend the cart array if it exists
    req.session.cart = req.session.cart || [];
    // add it if it does not exist
    if (!~req.session.cart.indexOf(req.params.id))
